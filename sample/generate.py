...
def main(args=None):
    ...
    # Only load the full dataset if we're using data files or action-to-motion
    # For text-only inference with just --text_prompt, skip dataset loading
    if is_using_data or args.action_file or args.action_name:
        print('Loading dataset...')
        data = load_dataset(args, max_frames, n_frames)
    else:
        # Create a minimal dummy dataset wrapper for text-only inference
        # This avoids loading the full TextOnlyDataset which requires split files
        print('Creating minimal dataset for text-to-motion inference...')
        
        # Load normalization constants for denormalization during inference
        mean_path = os.path.join('./dataset', 'HumanML3D', 'Mean.npy')
        std_path = os.path.join('./dataset', 'HumanML3D', 'Std.npy')
        mean = np.load(mean_path)
        std = np.load(std_path)
        
        class MinimalT2MDataset:
            """Minimal dataset wrapper for text-only inference"""
        ...
        class DummyDataset:
            """Minimal wrapper to provide necessary attributes for inference without loading dataset files"""
            def __init__(self, mean, std):
